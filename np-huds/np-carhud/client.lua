...
-- }
local gasStations = {
    {49.4187, 2778.793, 58.043,600},
    {263.894, 2606.463, 44.983,600},
    {1039.958, 2671.134, 39.550,900},
    {1207.26, 2660.175, 37.89996,900},
    {2539.685, 2594.192, 37.94488,1500},
    {2679.858, 3263.946, 55.24057,1500},
    {2005.055, 3773.887, 32.40393,1200},
    ...
    {620.8434, 269.1009, 103.0895,780},
    {2581.321, 362.0393, 108.4688,1500},
    {176.631, -1562.025, 29.263,1200},
    {-319.537, -1471.5116, 30.54118,600},
    {176.631, -1562.025, 29.263, 400},
    {1784.324, 3330.55, 41.253,600}
}
...
local seatbeltDisableExit = false
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsVehicleEngineOn(GetVehiclePedIsIn(player, false)) then          
            ...
            Fuel = ((100 * GetVehicleFuelLevel(vehicle) / GetVehicleHandlingFloat(vehicle,"CHandlingData","fPetrolTankVolume"))/154)*100
            -- print(Fuel)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if seatbeltDisableExit == true then
            ...
            if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
                ...
                if IsControlJustPressed(1, Controlkey["generalUse"][1]) then
                    if curFuel >= 100 then
                        ...
                        exports['mythic_notify']:SendAlert('error', "You are already full!")
                    else
                        Citizen.Wait(100)
